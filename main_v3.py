...
def _make_client_replay_buffers(
    number_of_clients, number_of_channels, history_length, batch_size, capacity
):
    """One private persistent replay memory per federated client.

    Client identity is the network index assigned by the simulator, so buffer
    `n` only ever holds transitions generated by network manager `n`.
    """
    return {
        client_id: _make_persistent_replay(
            number_of_channels=number_of_channels,
            history_length=history_length,
            batch_size=batch_size,
            capacity=capacity,
        )
        for client_id in range(int(number_of_clients))
    }
...
def run_federated_training(
    ...
    history = {
        ...
        "replay_buffer_counts_per_client": [],
        "stopped_early": False,
    ...
    if persistent_replay:
        client_replay_buffers = _make_client_replay_buffers(
            number_of_clients=max(max_nets, fixed_number_of_nets if fixed_topology else 0),
            number_of_channels=number_of_channels,
            history_length=history_length,
            batch_size=batch_size,
    ...
    for round_idx in range(communication_rounds):
        ...
        avg_reward, avg_channel_changes, agents, _ = worker(
            ...
            local_train_steps=local_train_steps,
            fedprox_mu=fedprox_mu,
            persistent_replay_buffer=client_replay_buffers,
            rho=rho,
            verbose=False,
        )

        per_client_counts = (
            {cid: int(buf.count) for cid, buf in client_replay_buffers.items()}
            if client_replay_buffers
            else {}
        )
        buffer_count = sum(per_client_counts.values())
        history["replay_buffer_counts"].append(buffer_count)
        history["replay_buffer_counts_per_client"].append(per_client_counts)
        ...
        if (round_idx + 1) % 10 == 0 or round_idx == 0:
            buffer_msg = (
                f" | replay_count={buffer_count} across {len(per_client_counts)}"
                f" clients (batch={batch_size})"
                if client_replay_buffers is not None
                else ""
            )
            print(
