...
spatially overlapping networks. The task is closed-loop and multi-agent:
each network manager must perceive local SINR quality, retain experience over
time, and select channels under partial observability, while coordinating with
peers that may not share raw sensing data. State-of-the-art deep reinforcement
learning (DRL) solutions for this setting operate under the \emph{centralized
training with decentralized execution} (CTDE) paradigm of
\CARLTON{}~\cite{cohen2024carlton}: although execution is distributed,
training requires every network to export its raw sensing history to a shared
replay memory and concentrates all gradient computation on a single learner.
We propose \OURS{}, a federated DRL framework that removes the centralized
learner. Each network manager acts as a federated client that owns a private,
persistent replay buffer (agent memory), performs $E$ local update steps of a
target-network-free mellowmax value learner, and uploads only model
parameters, which a server aggregates by \FedAvg{} or \FedProx{}. We identify
three sources of statistical heterogeneity specific to federated DRL for
DCA---topological asymmetry, policy-induced non-stationarity, and inter-agent
coupling---and show that the \FedProx{} proximal term keeps long local phases
stable under them. On the operational channel-quality metrics of the
centralized state of the art, evaluated over $420$ games per method, the
PyTorch CTDE baseline (\CARLTON{}) remains slightly ahead: all-sample
composite $\mathbb{E}[(\mathrm{CQ}+\min\mathrm{CQ})/2]$ of $0.909$ versus
$0.905$ for \FedProx{} and $0.902$ for \FedAvg{}, with weighted scores near
$0.931$ for all three. In-sample ($N\leq 7$) the gap shrinks to within metric
resolution ($0.983$ CTDE, $0.982$ \FedAvg{}, $0.978$ \FedProx{}). Thus
federation does \emph{not} outperform CTDE; it trades a thin margin for a
strictly stronger privacy constraint in which raw sensing data never leave the
network that produced them.
\end{abstract}

\begin{IEEEkeywords}
Agentic AI, multi-agent deep reinforcement learning, federated learning,
distributed dynamic channel allocation (DCA), privacy-preserving edge
intelligence, cognitive interference networks, inter-carrier interference
(ICI), SINR-aware resource management.
\end{IEEEkeywords}
...
mechanism in emerging cognitive communication systems. In the 6G edge regime,
independently managed networks must operate as closed-loop agents: they
perceive local spectrum quality, retain operational memory across time, reason
about channel decisions under uncertainty, and coordinate with neighbors under
strict latency and privacy constraints---capabilities that go beyond isolated
prediction or one-shot optimization. In contrast to static allocation, DCA lets
every network revise its operating channel in response to the interference it
currently experiences. The mechanism is relevant across cognitive radio
networks, 6G in-X sub-networks, vehicular ad-hoc networks, wireless sensor and
IoT networks, and wireless mesh networks---all characterized by dense
coexistence of independently managed networks within a shared
band~\cite{cohen2024carlton}.
...
Jamming Avoidance Response (JAR) heuristic used
in~\cite{cohen2024carlton}, outperforms the DRL baselines
...
the benchmark against which we position our framework; our goal is a
privacy-preserving federated counterpart that approaches---not
exceeds---this CTDE reference.
...
\textbf{C3. Privacy-preserving training that remains competitive with---but
does not surpass---CTDE.}
...
network managers. Federated clients train under a stricter information
constraint ($\rho=1$, no social term) than \CARLTON{} ($\rho=0.7$). On the
primary channel-quality metrics over $420$ games, PyTorch CTDE leads slightly
(all-sample composite $0.909$, in-sample $0.983$) while \FedProx{} /
\FedAvg{} attain $0.905$ / $0.902$ all-sample and $0.978$ / $0.982$
in-sample. The DeepMellow training reward is reported only as a learning
diagnostic; the claim of this paper is competitiveness under privacy, not
superiority over \CARLTON{}.

\textbf{C4. Agentic multi-agent DCA with private memory and
communication-efficient coordination.}
We cast each network manager as an autonomous agent with the four closed-loop
capabilities required for intelligent networks: \emph{perception} of
wideband BSINR/quality vectors, \emph{memory} via a persistent private replay
buffer retained across episodes, \emph{reasoning} via a mellowmax value policy
under partial observability, and \emph{coordination} via parameter-only
federation rather than raw experience pooling. This yields a trustworthy,
privacy-preserving form of multi-agent edge intelligence for SINR-aware
resource management, aligned with communication-efficient distributed learning
under non-IID local experience.
...
\label{alg:frl}
\begin{algorithmic}[1]
\Require Episodes $B$, local training steps $E$, proximal weight $\mu$, batch size
         $b_{z}$, buffer capacity $S_{z}$, schedule $\{\epsilon_{b}(t)\}$
\Ensure  Global policy parameters $w^{*}$
\State Server initializes $w^{0}$; each network manager $n$ initializes an empty
       persistent $\mathcal{D}^{n}$ of capacity $S_{z}$
\For{$t = 0,\dots,B-1$}
  \State Sample $N_{t}$ with $N_{t} < K$, generate a scenario,
         and sample participating clients $\mathcal{C}_{t}=\{1,\ldots,N_{t}\}$
  ...
  \For{$\tau = 1,\dots,T$}\Comment{interaction phase}
    \For{$n \in \mathcal{C}_{t}$ \textbf{in turn order}}
      ...
      \State Sample $a^{n}(\tau)$ by \eqref{eq:explore} with $\epsilon_{b}(t)$;
...
\begin{itemize}
\item \emph{\CARLTON{}} (PyTorch CTDE): centralized training with a shared
  global replay memory and $\rho=0.7$;
...
The pair $(E,\mu)=(10,0.01)$ is the outcome of a dense sweep over
$E\in\{1,2,5,10,15,20,40,60,80,120,200,300\}$ and a prior sweep over
$\mu\in\{0,0.001,0.01,0.1\}$ (Section~\ref{subsec:exp_reward}). Federated clients use
...
%------------------------------------------------------------------------------
\subsection{\texorpdfstring{\OURS{}'s Training Dynamics}{CARLTON FL's Training Dynamics}}
\label{subsec:exp_reward}
%------------------------------------------------------------------------------

We track the physical rate that the learned allocation actually delivers, rather
than treating the DeepMellow surrogate reward as the primary figure of merit.
For each saved global checkpoint we freeze the weights, play a fixed set of
in-sample scenarios, and convert the SINR each network achieves on its final
channel into a Shannon rate
\begin{equation}
R^{n}=B\log_{2}\!\bigl(1+\mathrm{SINR}^{n}\bigr),\qquad B=2~\mathrm{MHz},
\label{eq:shannon}
\end{equation}
averaged over the users of network $\mathcal{N}^{n}$.
Fig.~\ref{fig:throughput} reports the network-mean rate as a function of the
training round for \FedAvg{} and \FedProx{} (mean $\pm$ std over three seeds).
Panel~(a) shows the absolute Shannon throughput in Mbps; panel~(b) normalizes
each curve by its final-round value $R(T)$, so that the learning dynamics are
visible on a common scale. Both methods climb toward their operating point
within the first few hundred rounds and then stabilize, confirming that the
federated policies translate improving value estimates into higher delivered
throughput.

\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{campaign/throughput_vs_round.png}
\caption{Shannon throughput of the learned allocation.
(a)~Absolute rate $B\log_{2}(1+\mathrm{SINR})$ in Mbps
(light smoothing; faint lines show the raw mean).
(b)~Normalized learning curve $R(t)/R(T)$, where $T$ is the final training
round. Mean $\pm$ std over $3$ seeds; $B=2$~MHz.}
...
$E\in\{1,2,5,10,15,20,40,60,80,120,200,300\}$ for \FedProx{} ($\mu=0.01$)
and scores the final global model of each arm on composite channel quality
and Shannon throughput. The operating point used throughout the rest of the
paper is $E=10$, selected as the best trade-off on this dense grid; both much
shorter and much longer local phases lose measurable quality, consistent with
the bias--variance trade-off that \FedProx{} is designed to regularize.
...
$E$ over the dense grid
$\{1,2,5,10,15,20,40,60,80,120,200,300\}$.
Left: composite channel quality
...
Two remarks are important for interpretation. First, the DeepMellow training
reward is a surrogate used only during learning; the operational objectives of
interest are CQ, $\min$\,CQ, WS, and the Shannon rate of~\eqref{eq:shannon}
...
CTDE benefits from the social term ($\rho=0.7$). Under that stricter
constraint, matching or approaching---not exceeding---CTDE on channel-quality
metrics is the success criterion.
...
comparisons are: (i) \CARLTON{} itself (PyTorch CTDE), with and without the
post-processing threshold $\varphi$ that suppresses a channel switch unless it
improves CQ by at least $\varphi$; (ii) the Jamming Avoidance Response (JAR)
heuristic as implemented in~\cite{cohen2024carlton}, a bio-inspired scheme that
shifts the carrier by $\pm 2$~MHz only when the move improves CQ by at least
$0.05$, which is the state of the art for this model among non-learning
methods; (iii) the multi-agent DDQN allocator
MADDQN-DCA~\cite{adeogun2023distributed}; (iv) the deep recurrent RL scheme
IDSA~\cite{kaur2023deep}; and (v) the Random Agent
(RA)~\cite{cohen2017distributed}, which draws an initial
...
Finally, we caution against reading differences in the DeepMellow training
reward as evidence that federation is the stronger learner. Federated and
centralized runs optimize that surrogate under different $\rho$; the
operational claim of this paper rests on
Figs.~\ref{fig:fig15_bars}--\ref{fig:fig13_curves}, where CTDE
(\CARLTON{}) is slightly ahead overall and \OURS{} remains competitive under a
...
histories while retaining a fully decentralized execution phase. Each network
manager thus acts as a closed-loop agent with private memory and
parameter-only coordination---a privacy-preserving instance of multi-agent
edge intelligence for spectrum management. On the channel-quality metrics
$\mathbb{E}[(\mathrm{CQ}+\min\mathrm{CQ})/2]$ and WS, evaluated over $420$
games per method against RA, JAR, and a PyTorch CTDE (\CARLTON{}) baseline,
the centralized learner remains slightly ahead ($0.909$ versus $0.905$
all-sample composite for \FedProx{}; in-sample $0.983$ versus $0.978$), while
\OURS{} stays in the same regime without sharing raw sensing data. Federation
is therefore a practical path to data sovereignty in this setting: it does not
claim to outperform \CARLTON{}, but trades a thin margin against CTDE for a
strictly stronger privacy constraint.
%==============================================================================
...
\bibitem{du2022multi}
X.~Du, T.~Wang, Q.~Feng, C.~Ye, T.~Tao, L.~Wang, Y.~Shi, and M.~Chen,
