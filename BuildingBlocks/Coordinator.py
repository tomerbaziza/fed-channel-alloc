...
def _client_replay_buffer(persistent_replay_buffer, agent_id):
    """Resolve the private persistent replay memory of a federated client.

    `persistent_replay_buffer` is a mapping from client id to its own buffer.
    A single buffer object is still accepted for the non-federated callers that
    deliberately share one memory.
    """
    if isinstance(persistent_replay_buffer, dict):
        return persistent_replay_buffer.get(agent_id)
    return persistent_replay_buffer
...
def coordinator(environment, mutex = None, address_algo = '',training= True,
    ...
    # Merge episode transitions into the client's own persistent replay memory.
    # Each federated client keeps a private buffer, so no transition generated by
    # one network manager is ever visible to another.
    if training and persistent_replay_buffer is not None:
        for agent_id, agent in agents.items():
            client_buffer = _client_replay_buffer(persistent_replay_buffer, agent_id)
            if client_buffer is None:
                continue
            client_buffer.append_from_buffer(agent.experience_replay_buffer)
            agent.experience_replay_buffer = client_buffer
